...
the source scene's child paths available. Do not synthesize
`instance_placeholder` metadata manually; use it only when preserving
target-version output from a workflow that already depends on placeholders.
